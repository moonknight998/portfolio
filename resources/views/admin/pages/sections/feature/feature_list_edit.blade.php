            ...
            <div class="col-lg-12">
                <div class="card-group d-block d-md-flex row">
                    <div class="card col-md-7 p-2 mb-4">
                        <div class="card-header"><h2>{{__('admin/feature/feature.edit_feature')}}</h2></div>
                        <form method="POST" action="{{route('admin.feature_list.update', $feature_list_item->id)}}" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <div class="card-body">
                                <div class="example">
                                    ...
                                    <div class="tab-content rounded-bottom" id="content_tab">
                                      <div class="tab-pane p-3 active preview" role="tabpanel" >
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <input class="form-control" readonly id="icon" name="icon" type="text" onchange="changeAttribute(event, 'preview_icon', 'class')" value="bi bi-check" style="color: grey">
                                            @if ($errors->has('icon'))
                                                <div class="row mb-0">
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="form-select" id="status" name="status" onchange="changeStatus(event, 'status', 'can-hide')">
                                                <option {!!$feature_list_item->status == 1 ? 'selected' : ''!!} value="1">{{__('admin/common.display')}}</option>
                                                <option {!!$feature_list_item->status == 0 ? 'selected' : ''!!} value="0">{{__('admin/common.hide')}}</option>
                                            </select>
                                        </div>
                                      </div>
                                    </div>
                                                            ...
                                                            <div class="row-md-6 align-self-center gy-4" style="width: 100%">
                                                                @if (count($feature_lists) > 0)
                                                                    @foreach ($feature_lists as $feature_list_item_local)
                                                                        <div class="col-md-6 aos-init aos-animate py-1" data-aos="zoom-out" data-aos-delay="200" style="width: 100%; display: block">
                                                                            <div class="feature-box d-flex align-items-center" style="width: 100%; {{$feature_list_item_local->status == 0 ? 'background-color: rgba(187, 187, 187, 0.4)' : ''}}">
                                                                                <i id="{{$feature_list_item_local->id == $feature_list_item->id ? 'preview_icon' : ''}}" class="{{$feature_list_item_local->icon}}"></i>
                                                                                <h3 id="{{$feature_list_item_local->id == $feature_list_item->id ? 'preview_card_title' : ''}}">{{$feature_list_item_local->title}}</h3>
                                                                            </div>
                                                                        </div>
                                                                    @endforeach
                                                                @endif
