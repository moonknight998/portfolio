            ...
            <div class="col-lg-12">
                <div class="card-group d-block d-md-flex row">
                    <div class="card col-md-7 p-2 mb-4">
                        ...
                        <form method="POST" action="{{$value_title == null ? route('admin.value_title.store') : route('admin.value_title.update', $value_title->id)}}" enctype="multipart/form-data">
                            @csrf
                            @if ($value_title)
                            @method('PATCH')
                            @endif
                                    ...
                                    <div class="tab-content rounded-bottom" id="content_tab">
                                      <div class="tab-pane p-3 active preview" role="tabpanel" >
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <textarea class="form-control" rows="5" id="title" name="title" type="text" placeholder="{{__('admin/value/value-index.title_placeholder')}}"
                                            onchange="loadDocument(event, 'preview_title')"
                                            onkeypress="detectEnterline(event, 'title'); loadDocument(event, 'preview_title')"
                                            >{{$value_title ? ($value_title->title === '' ? old('title') : $value_title->title) : ''}}</textarea>
                                            @if ($errors->has('title'))
                                                <div class="row mb-0">
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="form-select" id="status" name="status">
                                                <option {!!$value_title ? ($value_title->status == 1 ? 'selected' : '') : 'selected'!!} value="1">{{__('admin/common.display')}}</option>
                                                <option {!!$value_title ? ($value_title->status == 0 ? 'selected' : '') : ''!!} value="0">{{__('admin/common.hide')}}</option>
                                            </select>
                                        </div>
                                      </div>
                                    </div>
