    ...
    <div class="container-fluid m-1">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb my-0 ms-2">
            ...
            <li class="breadcrumb-item active"><a>{{__('admin/client/client.edit_item')}}</a></li>
          </ol>
        </nav>
      </div>
</header>
                ...
                <div class="card-group d-block d-md-flex row">
                    <div class="card col-md-7 p-2 mb-4">
                        <div class="card-header">
                            <h2>{{__('admin/client/client.edit_item')}}</h2>
                        </div>
                        <form method="POST" action="{{route('admin.client_item.update', $client_item->id)}}" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <div class="card-body">
                                <div class="example">
                                    ...
                                    <div class="tab-content rounded-bottom" id="content_tab">
                                      <div class="tab-pane p-3 active preview" role="tabpanel" >
                                        ...
                                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                                            {{__('admin/client/client.item_updated')}}
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            onchange="loadDocument(event, 'preview_brand_name')" value="{{ShowFormValue($client_item, "brand_name")}}"></input>
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="form-select" id="status" name="status">
                                                <option {{$client_item->status == 1 ? 'selected' : ''}} value="1">{{__('admin/common.display')}}</option>
                                                <option {{$client_item->status == 0 ? 'selected' : ''}} value="0">{{__('admin/common.hide')}}</option>
                                            </select>
                                        </div>
                                      </div>
                                    </div>
                                            ...
                                            <section id="clients" class="clients">
                                                <div class="container" data-aos="fade-up">
                                                    ...
                                                    <div class="clients-slider swiper">
                                                        <div class="{{count($client_items) > (MobileDetect()->isMobile() ? 1 : 5) ? "swiper-wrapper align-items-center" : "container-fluid little-item"}}">
                                                            @if (count($client_items) > 0)
                                                                @foreach ($client_items as $client_item_local)
                                                                    @if ($client_item_local->id == $client_item->id)
                                                                        <div class="{{count($client_items) > (MobileDetect()->isMobile() ? 1 : 5) ? "swiper-slide" : "image-container"}}"><img id="preview_logo" src="{{$client_item->logo}}" class="img-fluid" alt=""></div>
                                                                    @else
                                                                        <div class="{{count($client_items) > (MobileDetect()->isMobile() ? 1 : 5) ? "swiper-slide" : "image-container"}}"><img src="{{$client_item_local->logo}}" class="img-fluid" alt="{{$client_item_local->brand_name}}"></div>
                                                                    @endif
                                                                @endforeach
                                                            @endif
                                                        </div>
